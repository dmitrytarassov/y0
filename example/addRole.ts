...
const [roleId, account, network] = process.argv.reverse();
...
async function addRole(_account, role) {
  // @ts-ignore
  console.log(`Use ${web3.currentProvider.host}`);
  console.log(`Add role ${role} to account ${_account}`);
  ...
  const extraData = await contract.methods.grantRole(_account, role);
